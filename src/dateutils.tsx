...
export const getMoveParms = (
  ...
  const periodLabel = stepLabels[stepValue as keyof typeof stepLabels];
...
export const getInitRange = (
  ...
  const _rnge = {
    ...
    lastDateOfScope: { start: rangeScope.end, end: rangeScope.end },
    days5FromToday: day(0, subDays(startOfToday(), 4)),
    days6FromToday: day(0, subDays(startOfToday(), 5)),
    days7FromToday: day(0, subDays(startOfToday(), 6)),
    days8FromToday: day(0, subDays(startOfToday(), 7)),
  };
  return rtn !== ""
    ? _rnge
    : _rnge[startRange as keyof typeof _rnge]
      ? _rnge[startRange as keyof typeof _rnge]
      : rangeScope;
};
...
export const getRange = (fn: string, step: string, dates: dateRange) => {
  const [start, end] = moveFns[step as keyof typeof moveFns]?.(dates)[
    fn as keyof ReturnType<(typeof moveFns)["day"]>
  ];
...
export const Increment = (
  ...
  const _rnge = getInitRange(
    ...
    weekStartDay,
    yearStartMonth,
    { start: null, end: null } as unknown as dateRange,
    "matrix",
...
function eachPaydayOfInterval(range: dateRange, pay: { ref: Date; len: number }) {
...
export const createMarks = (
  ...
  const result = periodHandlers[period as keyof typeof periodHandlers]
    ? periodHandlers[period as keyof typeof periodHandlers]()
    : [];
...
const buildMarks = (period: string, props: SliderProps, locale: any) => {
  if (!props.rangeScope || !props.stepFmt || !props.stepSkip) {
    return [];
  }
  ...
  const fmt = props.stepFmt[period as keyof typeof props.stepFmt];
  const step = props.stepSkip[period as keyof typeof props.stepSkip];
  const offset = period === "year" ? (props.yearStartMonth ?? 0) : 0;
  return doMarks(marks, fmt, props.rangeScope.start, step, offset, locale);
};

export const mainMarks = (props: SliderProps, locale: any) => {
  if (!props.stepValue) return null;
  const _val = stepMajor[props.stepValue as keyof typeof stepMajor];
  if (_val != null) {
...
export const superMarks = (props: SliderProps, locale: any) => {
  if (!props.stepValue) return null;
  const _val = stepMinor[props.stepValue as keyof typeof stepMinor];
  if (_val != null) {
    return buildMarks(_val, props, locale);
  }
  return null;
