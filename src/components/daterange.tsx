...
export default function DateRange(props: dateCardProps) {
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Commit immediately when a complete valid date arrives (e.g. from the browser calendar popup).
    // onBlur only fires when the input loses focus, which doesn't happen after a picker selection.
    const dte = parse(value, "yyyy-MM-dd", new Date());
    if (isValid(dte)) {
      doUpdate(id as "start" | "end", value);
    }
  };
